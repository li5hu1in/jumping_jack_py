from flask import Flask, render_template, request, flash, redirect, url_for, send_from_directory, Response, jsonify
...
def upload_video():
    ...
    try:
        ...
        # Process the video using your Python code
        process_video(f"{video_file.filename}")

        return render_template('index.html', result=os.path.exists("static/output.mp4"), scroll_to_button=True)

    except Exception as e:
        flash(f"Error when processing video: {str(e)}", "error")
        return redirect(url_for('index'))

# download output when finish processing
@app.route('/download_output')
def download_output():
    try:
        output_file_path = "static/output.mp4"
        return send_from_directory(os.path.dirname(output_file_path), os.path.basename(output_file_path), as_attachment=True)
    except Exception as e:
...
def video_stream():
    return Response(realtime_counter(), mimetype='multipart/x-mixed-replace; boundary=frame')
