...
from main import FullBodyPoseEmbedder, PoseClassifier, EMADictSmoothing, RepetitionCounter, PoseClassificationVisualizer
...
def realtime_counter():
    ...
    # 指定视频路径和输出名称
    # update_count(repetitions_count, reset=True)
    class_name = "up"

    video_cap = cv2.VideoCapture(0)
    video_cap.set(cv2.CAP_PROP_FRAME_WIDTH, 0)
    video_cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 0)
    ...
    while True:
        ...
        if pose_landmarks is not None:
            ...
            if num_visible_landmarks != 33:
                ...
                warning_text = "请全身入画"
                warning_font = ImageFont.truetype("fonts/msyh.ttc", 50)
        ...
        _, jpeg = cv2.imencode('.jpg', cv2.cvtColor(np.array(output_frame), cv2.COLOR_RGB2BGR))
        frame = jpeg.tobytes()
        yield (b'--frame\r\n'
